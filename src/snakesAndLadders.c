...
void readBoard(cell board[][8]);
void changeCharacter(char *plyr);
...
int main() {
  ...
  int i, j, k, l, r, state, winners, choice, mode;
  choice = 0;
  mode = 0;
  winners = 0;
  ...
  while (choice != 5) {
    ...
    printf("  / __/__  ___ _/ /_____ ___   ___ ____  ___/ / / /  ___ ____/ /__/ /__ _______\n");
    printf(" _\\ \\/ _ \\/ _ `/  '_/ -_|_-<  / _ `/ _ \\/ _  / / /__/ _ `/ _  / _  / -_) __(_-<\n");
    printf("/___/_//_/\\_,_/_/\\_\\\\__/___/  \\_,_/_//_/\\_,_/ /____/\\_,_/\\_,_/\\_,_/\\__/_/ /___/\n\n");
    printf("\t\t\tVersion 4.07\t\tGoSanchezSy\n\n");
    printf("\t\t\t\t1> Start Game\n");
    printf("\t\t\t\t2> Reload board.txt\n");
    printf("\t\t\t\t3> Change Character\n");
    if (mode == 0) {
      printf("\t\t\t\t4> Mode: Manual\n");
    } else if (mode == 1) {
      printf("\t\t\t\t4> Mode: Automatic\n");
    }
    printf("\t\t\t\t5> Exit\n\n");
    printf("\t\t\tChoice: ");
    scanf("%d", &choice);
    ...
    switch(choice) {
      ...
      board[7][0].players[1][0] = (char)238;
      board[7][0].players[0][1] = (char)239;
      board[7][0].players[1][1] = (char)229;
...
/* function to initialize the character selection screen*/
void changeCharacter(char *plyr) {
  int choice;
  for (;;) {
    choice = 0;
...
/* function to read board.txt and update the board in-program*/
void readBoard(cell board[][8]) {
  int i, j, k, l;
  char c, temp[2];
...
void printBoard(cell board[][8], char blah) {
  ...
  printf("Snake Tails: T#\t\tPlayer 2: %c\n", 238);
  printf("Ladder Bottoms: B#\tPlayer 3: %c\n", 239);
  printf("Ladder Ends: E#\t\tPlayer 4: %c\n", 229);
  printf("\n");
}
